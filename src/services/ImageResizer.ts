...
import ResizeOptions from '../model/interfaces/ResizeOptions';
...
export default class ImageResizer {
    ...
    public static createImageDataURLFromFile(file: File): Promise<HTMLImageElement> {
        ...
        return new Promise<HTMLImageElement>((resolve, reject) => {
            ...
            reader.onload = async (e: any) => {
                ...
                const exif = EXIF.readFromBinaryFile(ImageResizer.base64ToArrayBuffer(e.target.result));
    ...
    public static resizeImage(image: HTMLImageElement, resize: ResizeOptions): HTMLImageElement {
        const dimensions = ImageResizer.calculateScaleDimensions(image.width, image.height, resize.width, resize.height);
        const scaledCanvas = ImageResizer.downscaleImage(image, dimensions.ratio);

        const scaledImage = document.createElement('img');
        scaledImage.src = scaledCanvas.toDataURL(resize.mimeType || 'image/WebP', resize.quality ? resize.quality / 100 : 0.934);
        return scaledImage;
    }

    public static getImageFileSize(image: HTMLImageElement) {
        const base64: string = image.src.replace(/^data:([^;]+);base64,/gmi, '');
    ...
    private static calculateScaleDimensions(srcWidth, srcHeight, maxWidth, maxHeight): { ratio: number, width: number, height: number } {
    ...
    private static downscaleCanvas(canvas: HTMLCanvasElement, context: CanvasRenderingContext2D, scale: number): HTMLCanvasElement {
        if (!(scale < 1) || !(scale > 0)) {
        ...
        let pxIndex = 0; //
        for (sourceIndex = 0, targetYIndex = 0; pxIndex < targetWidth * targetHeight; sourceIndex += 3, targetYIndex += 4, pxIndex++) {
